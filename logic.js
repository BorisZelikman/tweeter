const Tweeter = function () {
  ...
  const _initCounters = () => {
    ...
    for (const post of _posts) {
      ...
      for (const comment of post.comments) {
        const commentNum = _idNum(comment.id, _commentIdCounter.prefix); // post.id
        if (commentNum > maxCommentNum) maxCommentNum = commentNum;
      }
    }
    _postIdCounter.count = maxPostNum;
    _commentIdCounter.count = maxCommentNum; // -
  };
  ...
  const removePost = (id) => {
    ...
    if (postIndex >= 0) _posts.splice(postIndex, 1); // bug >=
  };

  const addComment = (commentText, postId) => {
    if (_postIdCounter.count === 0) _initCounters(); // .count
